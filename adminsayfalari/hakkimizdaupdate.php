...
<?php
        $id=$_GET['id'];
        $sorgu=$DBcon ->prepare("SELECT * FROM hakkimizda WHERE id=:id");
...
?>
    ...
    <div class="con">  
      <form id="contt" action="" method="POST" enctype="multipart/form-data">
       <center> <h3>Düzenle</h3>
        <h4>Hakkımızda Düzenle</h4></center>
          
          
          <fieldset>
          Hakkımızda Başlık: <input type="text" name="baslik" value="<?php echo $row['baslik']; ?>" required>
          </fieldset>
          
          
          
          <fieldset>
              Hakkımızda Alt Başlık: <textarea type="text" name="altbaslik" id="projedetay"  ><?php echo $row['altbaslik']; ?></textarea>
          </fieldset>
    ...
    <?php 
            $uyesor = $DBcon->prepare("select * from hakkimizda where id=:id");
    ...
    unlink("../img/hakkimizda/".$eskiresim);
    ...
    move_uploaded_file($tmp_name, "$yukleklasor/$resimad");
            ...
            $guncelle= $DBcon -> prepare("UPDATE hakkimizda SET resim=:resim, baslik=:baslik, altbaslik=:altbaslik WHERE id=:id");
