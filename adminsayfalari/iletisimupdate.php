...
<?php
        $id=$_GET['id'];
        $sorgu=$DBcon ->prepare("SELECT * FROM iletisim WHERE id=:id");
...
?>
    ...
    <div class="con">  
      <form id="contt" action="" method="POST" enctype="multipart/form-data">
       <center> <h3>Düzenle</h3>
        <h4>İletişim Düzenle</h4></center>
          
          
          <fieldset>
          İletişim Başlık: <input type="text" name="baslik" value="<?php echo $row['baslik']; ?>" required>
          </fieldset>
          
          
          
          <fieldset>
              İletişim Alt Başlık: <textarea type="text" name="altbaslik" id="projedetay"  ><?php echo $row['altbaslik']; ?></textarea>
          </fieldset>
          ...
          <fieldset>
          İnput 1 : <input type="text" name="input1" value="<?php echo $row['input1']; ?>" required>
          </fieldset>

           
          <fieldset>
          İnput 2 : <input type="text" name="input2" value="<?php echo $row['input2']; ?>" required>
          </fieldset>

           
          <fieldset>
          İnput 3 : <input type="text" name="input3" value="<?php echo $row['input3']; ?>" required>
          </fieldset>

           
          <fieldset>
          İnput 4 : <input type="text" name="input4" value="<?php echo $row['input4']; ?>" required>
          </fieldset>
    ...
    <?php 
    $uyesor = $DBcon->prepare("select * from iletisim where id=:id");
    ...
    move_uploaded_file($tmp_name, "$yukleklasor/$resimad");
            ...
            $guncelle= $DBcon -> prepare("UPDATE iletisim SET baslik=:baslik, altbaslik=:altbaslik, input1=:input1, input2=:input2, input3=:input3, input4=:input4, resim=:resim WHERE id=:id");
            $guncelle -> execute(array(":baslik" => $baslik,":altbaslik" => $altbaslik,":input1" => $input1,":input2" => $input2,":input3" => $input3,":input4" => $input4,":resim" => $resimad,":id" =>$id));
            if($guncelle){
                echo "<script type='text/javascript'>alert('Başarıyla Güncellendi');</script>";
                header("Refresh:0 ; url=iletisimduzenle.php");
            }
            else
            {
