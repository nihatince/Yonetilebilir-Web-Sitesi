...
<?php
        $id=$_GET['id'];
        $sorgu=$DBcon ->prepare("SELECT * FROM lokasyonbaslik WHERE id=:id");
...
?>
    ...
    <div class="con">  
      <form id="contt" action="" method="POST">
       <center> <h3>Düzenle</h3>
        <h4>Anasayfa Düzenle</h4></center>
          
          
          <fieldset>
          Lokasyon Başlık: <input type="text" name="baslik" value="<?php echo $row['baslik']; ?>" required>
          </fieldset>
          
          
          
          <fieldset>
              Lokasyon Alt Başlık: <textarea type="text" name="altbaslik" id="projedetay"  ><?php echo $row['altbaslik']; ?></textarea>
          </fieldset>
    ...
    <?php 
        ...
        if($_POST){
            ...
            $guncelle= $DBcon -> prepare("UPDATE lokasyonbaslik SET baslik=:baslik, altbaslik=:altbaslik WHERE id=:id");
            $guncelle -> execute(array(":baslik" => $baslik,":altbaslik" => $altbaslik,":id" =>$id));
            if($guncelle){
