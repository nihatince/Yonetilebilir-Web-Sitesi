...
<?php
        $id=$_GET['id'];
        $sorgu=$DBcon ->prepare("SELECT * FROM odalarimiz WHERE id=:id");
...
?>
      ...
      <form id="contt" action="" method="POST" enctype="multipart/form-data">
       <center> <h3>Düzenle</h3>
        <h4>Anasayfa Düzenle</h4></center>
          ...
          <fieldset>
          Odalarımız Başlık: <input type="text" name="baslik" value="<?php echo $row['baslik']; ?>" required>
          </fieldset>
          
          
          
          <fieldset>
              Odalarımız Alt Başlık: <textarea type="text" name="altbaslik" id="projedetay"  ><?php echo $row['altbaslik']; ?></textarea>
          </fieldset>
    ...
    <?php 
    $uyesor = $DBcon->prepare("select * from odalarimiz where id=:id");
    ...
    // dosya varmı kontrol
    if(strlen($name) > 0){
        $yukleklasor = '../img/anasayfa/icerik';
    ...
    unlink("../img/anasayfa/icerik/".$eskiresim);
    ...
    move_uploaded_file($tmp_name, "$yukleklasor/$resimad");
            ...
            $guncelle= $DBcon -> prepare("UPDATE odalarimiz SET resim=:resim, baslik=:baslik, altbaslik=:altbaslik WHERE id=:id");
            ...
            if($guncelle){
                ...
                header("Refresh:0 ; url=anasayfaduzenle.php");
            }
            else
            {
