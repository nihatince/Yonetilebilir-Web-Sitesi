	 ...
	 <div class="container pt-5 pb-5">        
      <table class="table"> 
       ...
       <tbody>
        ...
        $query = $DBcon->query('SELECT * FROM iletisim ORDER BY id ASC',PDO::FETCH_ASSOC);
          ...
          foreach($query as  $row){
            ...
            ?>
              ...
              <td data-target="input1" ><?=$row['input1']?></td>
              <td data-target="input2" ><?=$row['input2']?></td>
              <td data-target="input3" ><?=$row['input3']?></td>
              <td data-target="input4" ><?=$row['input4']?></td>
              <?php
                if($row['resim']=="")
                {
                    ?>
              ...
              {
                ?>
              <td data-target="resim" ><img width="175px" height="100px" src="../img/iletisim/<?=$row['resim']?>"></td>
<?php
              }
                    ?>
            
          
                    <td><a href="iletisimupdate.php?id=<?php echo $row['id']?>"><i class="fa-solid fa-pen-to-square" style='font-size:27px'></i></a></td>

                  <td>
                    
                        <label class="switch">
                        <!-- checkbox a id ve checked bilgilerini ekliyoruz -->
                        <input type="checkbox" id='<?php echo $row['id'] ?>' class="aktifPasif" <?php echo $row['buton']==1?'checked':'' ?>  />
   ...
   <script>
    ...
    $('.aktifPasif').click(function (event) {
        ...
        var buton = ($(this).is(':checked')) ? '1' : '0';
        //checkbox a göre aktif mi pasif mi bilgisini alıyoruz.

        $.ajax({
            type: 'POST',
            url: 'aktifpasif2.php',  //işlem yaptığımız sayfayı belirtiyoruz
            data: { id:id,buton: buton }, //datamızı yolluyoruz
