...
<?php
        $id=$_GET['id'];
        $sorgu=$DBcon ->prepare("SELECT * FROM konaklama WHERE id=:id");
...
?>
      ...
      <form id="contt" action="" method="POST" enctype="multipart/form-data">
       <center> <h3>Düzenle</h3>
        <h4>Anasayfa Düzenle</h4></center>
          ...
          <fieldset>
          Konaklama Başlık: <input type="text" name="baslik" value="<?php echo $row['baslik']; ?>" required>
          </fieldset>
          
          
          
          <fieldset>
              Konaklama Alt Başlık: <textarea type="text" name="altbaslik" id="projedetay"  ><?php echo $row['altbaslik']; ?></textarea>
          </fieldset>
    ...
    <?php 
     $uyesor = $DBcon->prepare("select * from konaklama where id=:id");
    ...
    unlink("../img/konaklama/".$eskiresim);
    ...
    move_uploaded_file($tmp_name, "$yukleklasor/$resimad");
            ...
            $guncelle= $DBcon -> prepare("UPDATE konaklama SET resim=:resim, baslik=:baslik, altbaslik=:altbaslik WHERE id=:id");
            ...
            if($guncelle){
                ...
                header("Refresh:0 ; url=konaklamaduzenle.php");
            }
            else
            {
